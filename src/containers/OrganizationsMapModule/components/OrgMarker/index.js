...
const OrgMarker = ({
  ...
  return (
    <Marker position={position} icon={customMarker} onClick={onClick}></Marker>
  );
};
...
export const OrgMarkerView = ({ org, showAverageCheck = false, ...rest }) => {
  return (
    <div
      className="organization-map-module__org-marker"
      {...rest}
      role="button"
    >
      ...
      {org.avg_check !== null && showAverageCheck && (
        <div className="organization-map-module__org-marker-avg-check f-14 tl">
          {prettyMoney(org.avg_check).replaceAll("'", ",")}{" "}
          <span className="f-500">{org.currency}</span>
        </div>
      )}
    </div>
  );
