...
import {getLikedPosts} from '../../store/services/postServices';
import PostFeedCard from '../../components/Cards/PostFeedCard';
import Preloader from '../../components/Preloader';
import {DEFAULT_LIMIT} from '../../common/constants';
import {translate} from '../../locales/locales';
import EmptyImage from '../../assets/images/empty_liked_feed.png';
...
class LikedPostsPage extends Component {
  ...
  componentDidMount() {
    getLikedPosts(this.state.params)
  ...
  getNext = totalPages => {
    ...
    if (params.page < totalPages) {
      ...
      return getLikedPosts({...params, page: nextPage})
  ...
  render() {
    ...
    return (
      <div className="liked-posts-page">
        <MobileTopHeader
          title={translate("Понравившиеся", "app.liked")}
          onBack={() => canGoBack(history) ? history.goBack() : history.push(`/profile`)}
        />
        <div className="liked-posts-page__content">
          {(page === 1 && loading)
            ? <Preloader />
            : hasPosts && (
            <div className="liked-posts-page__list">
              <InfiniteScroll
              ...
              >
                {posts.list.map(post => (
                  <PostFeedCard
                    ...
                    isGuest={!user}
                    className="liked-posts-page__card"
                  />
                ))}
              </InfiniteScroll>
            </div>
          ...
          {!loading && !hasPosts && (
            <div className="liked-posts-page__empty">
              <Empty
                label={translate("У вас пока нет понравившихся", "shop.liked.empty")}
                image={EmptyImage}
              />
            </div>
          )}
...
export default withScroll(LikedPostsPage);