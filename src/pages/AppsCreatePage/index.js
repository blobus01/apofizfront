    ...
    try {
      ...
      if (
        ...
        const payload = {
          ...
          banners_image_ids: values.tempBanners.map(
            (banner) => banner.originalImage.id
          ),
          selected_banner_file_id: values.banner?.originalImage?.id || [],
        ...
        const appResponse = await axios.post("/applications/", payload);
        if (appResponse.status === 201 || appResponse.status === 200) {
          Notify.success({
            text: translate("Приложение успешно создано", "apps.createSuccess"),
          });
          history.push("/apps/" + appResponse.data.id);
        } else {
          Notify.error({
            text: translate(
      ...
      Notify.error({
        text: translate("Ошибка при создании приложения", "apps.createError"),
      });
    } finally {
      setSubmitting(false);
    }
    ...
    >
      {(formikBag) => {
        ...
        const validateState =
          ...
          values.image?.id &&
          values.tempBanners?.length > 0 &&
          values.banner?.originalImage?.id &&
          values.app_link;
        return (
          <div className="apps-create-page">
            {values.step === 0 && (
              <form onSubmit={handleSubmit}>
                <MobileTopHeader
                  title={translate("Добавить приложение", "apps.addApp")}
                  ...
                  renderRight={() => (
                    ...
                    >
                      {isSubmitting
                        ? translate("Создание...", "apps.creating")
                        : translate("Добавить", "apps.create")}
                    </button>
                  )}
                />
                <div
                ...
                <div className="container apps-create-form">
                  ...
                  <div className="form-section">
                    ...
                    <div className="form-section__input-fields">
                      <InputTextField
                        ...
                        hint={translate(
                          "Обязательно добавьте название приложения",
                          "apps.nameHint"
                        )}
                      />
                      <InputTextField
                      ...
                      {/* Для фото приложения используйте ваш компонент загрузки изображений, например: */}
                      <ImagesUpload
                        name="photos"
                        value={values.photos}
                        onChange={handleChange}
