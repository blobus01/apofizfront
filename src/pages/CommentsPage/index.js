...
import { MarketIcon, MenuDots, PhoneCallIcon } from "@components/UI/Icons";
...
const CommentsPage = ({ match, isChat = false, history }) => {
  ...
  const isAssistantWorking = isChat;

  const chatSocket = useChatSocket(chatOrItemID, {
    connect: isChat || (!isChat && canComment),
    isChat,
  });
  ...
  useEffect(() => {
    chatSocket.onMessage((e) => {
      try {
        ...
        if (newMessage.user === null && newMessage.assistant !== null) {
  ...
  const onSubmit = async (values) => {
    if (
      ...
      values.text.trim().startsWith("http")
    ) {
      return;
    }
  ...
  // для сброса состояния "ассистент печатает" при получении нового сообщения от ассистента
  useEffect(() => {
    ...
    <div className="post-comments-page commentsPage">
      {ai === "active" ? (
        ...
        <MobileTopHeader
          ...
          className="post-comments-page__top-header"
          renderRight={() => (
            <div className="post-comments-page__right-side">
              {isCommentsPostPage ? (
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      <div
                        className={classNames(
                          "post-comments-page__cart",
                          showCartNotification &&
                            "post-comments-page__cart--visible",
                          allCartsTotalCount > 99 &&
                            "post-comments-page__cart--count-more",
                        )}
                      >
                        <span>
        ...
        (!isChat || chatID !== setChat?.id) && (
          ...
          >
              ...
              >
                ...
                >
                  {comments.list.map((comment) => {
                    ...
                    const isAiMessage =
                      comment.user === null && comment.assistant !== null;
                    const isUserMessage = comment.user !== null;
            ...
            <div className={classNames("post-comments-page__controls")}>
              ...
              {canEdit && isChat && comments?.list?.[0]?.source === "web" && (
